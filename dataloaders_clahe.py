...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.stride = stride
        self.path = path
        self.albumentations = get_albumentations(hyp, img_size) if augment else None
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Albumentations
            if self.albumentations and labels.size:
                class_labels = labels[:, 0].copy()
                bboxes = labels[:, 1:]
                sample = self.albumentations(image=img, bboxes=bboxes, class_labels=class_labels)
                img = sample['image']
                if len(sample['bboxes']):
                    labels = np.concatenate([
                        np.array(sample['class_labels']).reshape(-1, 1),
...
def get_albumentations(hyp, size=640):
    import albumentations as A
    return A.Compose([
